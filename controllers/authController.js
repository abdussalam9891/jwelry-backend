  ...
  try {
    ...
    if (user) {
      ...
      return res.status(200).json({
        success: true,
        message: "Email login linked. Verify OTP to continue.",
        email,
      });
    }
...
// GOOGLE SUCCESS
export const googleAuthSuccess = (req, res, redirectUrl) => {
  const user = req.user;
  ...
  if (req.session) {
    req.session.destroy((err) => {
      ...
      return res.redirect(redirectUrl); // ONLY RESPONSE
    });
...
// ADMIN CALLBACK
export const handleAdminGoogleCallback = (req, res) => {
...
// USER CALLBACK
export const handleUserGoogleCallback = (req, res) => {
  googleAuthSuccess(
    req,
...
export const firebaseLogin = async (req, res) => {
  try {
    ...
    res.cookie("user_token", token, USER_COOKIE);
